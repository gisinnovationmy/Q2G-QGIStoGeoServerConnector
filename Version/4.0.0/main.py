...
from qgis.PyQt.QtWidgets import (
    ...
    QSplitter, QListWidget, QCheckBox, QTreeWidget, QHeaderView, QProgressBar,
    QMessageBox, QFileDialog, QAbstractItemView, QSizePolicy, QGroupBox, QLabel,
    QMenu, QInputDialog, QWidget, QAction, QShortcut
)
from qgis.core import QgsSettings
from qgis.PyQt.QtGui import QKeySequence, QIcon, QPixmap
...
try:
    ...
    if os.path.isdir(_candidate) and os.path.exists(os.path.join(_candidate, 'main.py')):
...
# Load import_manager from local file
_im = _load_local_module("import_manager")
dynamic_import = _im.dynamic_import
get_import_manager = _im.get_import_manager
...
AllDuplicateStoresForLayerCleaner = dynamic_import(
    "cleanup_all_duplicate_stores_for_layer", "AllDuplicateStoresForLayerCleaner")
...
GeoPackageLayerExistenceChecker = dynamic_import(
    "geopackage_layer_exists_in_datastore", "GeoPackageLayerExistenceChecker")
...
class QGISGeoServerLayerLoader(QDialog):
    ...
    def _show_custom_notification(self, message):
        ...
        try:
            ...
            label.setFont(QFont("Arial", 11, QFont.Weight.Normal))
            ...
            print("DEBUG: Notification dialog shown successfully")
    ...
    def _setup_window_and_styling(self):
        ...
        try:
            plugin_dir = _CURRENT_DIR
            logo_path = os.path.join(plugin_dir, 'images', 'logo.svg')
        ...
        self.setStyleSheet('''
            ...
            QGroupBox { font-weight: bold; border: 1px solid #bbb; border-radius: 6px;
                        margin-top: 8px; background: #fcfcfc; }
            QGroupBox::title { subcontrol-origin: margin; left: 10px; padding: 0 5px 0 5px; }
            QLabel { font-size: 10pt; }
            QLineEdit, QComboBox, QListWidget, QTreeWidget { font-size: 10pt; background: #fff;
                        border: 1px solid #ccc; border-radius: 3px; }
            QPushButton { font-size: 10pt; padding: 6px 16px; background: #e6e6e6;
            ...
            QPushButton#delete, QPushButton#delete_style_btn, QPushButton#delete_workspace_btn {
    ...
    def _setup_connection_section(self, main_layout):
        ...
        self.test_paths_btn = QPushButton("Test (Debug Paths)")
        self.test_paths_btn.clicked.connect(self.show_debug_paths_dialog)
        self.test_paths_btn.setVisible(False)  # Hidden by default, shown in debug mode
        others_layout.addWidget(self.test_paths_btn)
    ...
    def _setup_workspaces_section(self, top_splitter):
        ...
        self.workspaces_list.setMinimumWidth(int(self.workspaces_list.sizeHintForColumn(
            0) * 1.3) if self.workspaces_list.sizeHintForColumn(0) > 0 else 200)
        ...
        self.workspaces_list.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        ...
        self.delete_workspace_btn.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
    ...
    def _setup_qgis_layers_section_in_panel(self, qgis_layout):
        ...
        self.auto_overwrite_checkbox.setToolTip(
            "When checked, existing layers will be automatically overwritten without asking")
        ...
        # Cased line styles checkbox (for rule-based line layers)
        self.cased_lines_checkbox = QCheckBox("Cased line styles")
        self.cased_lines_checkbox.setToolTip(
            "When checked, line rules will be rendered as GeoServer cased lines "
            "(border drawn behind fill) for continuous intersections"
        )
        self.cased_lines_checkbox.setChecked(False)
        top_row.addWidget(self.cased_lines_checkbox)
        ...
        self.qgis_layers_tree.setColumnCount(6)
        self.qgis_layers_tree.setHeaderLabels(["Name", "Symbol", "Format", "Properties ", "Show SLD", "Upload SLD"])
        self.qgis_layers_tree.setAlternatingRowColors(True)
        self.qgis_layers_tree.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        ...
        self.qgis_layers_tree.header().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.qgis_layers_tree.header().setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        self.qgis_layers_tree.header().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)  # Rule Based
        self.qgis_layers_tree.header().setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)
        self.qgis_layers_tree.header().setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)
        self.qgis_layers_tree.header().setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents)
        self.qgis_layers_tree.header().setStretchLastSection(False)

        qgis_inner_layout.addWidget(self.qgis_layers_tree)

        # Symbol column legend (explains the M / S indicators)
        self.symbol_legend_label = QLabel(
            "<b>Symbol column:</b> "
            "<span style='color:#006600;'><b>S</b></span> = single-layer symbol rule-based &nbsp;&nbsp; "
            "<span style='color:#006600;'><b>M</b></span> = multi-layer symbol rule-based "
            "(flattened into one draw pass per symbol layer) &nbsp;&nbsp; "
            "blank = standard symbology"
        )
        self.symbol_legend_label.setStyleSheet("font-size: 9pt; color: #444;")
        self.symbol_legend_label.setWordWrap(True)
        qgis_inner_layout.addWidget(self.symbol_legend_label)
        ...
        self.mini_test_btn = QPushButton("🧪 Mini WebEngine Test")
        self.mini_test_btn.setToolTip("Open minimal WebEngine test dialog to diagnose issues")
        self.mini_test_btn.clicked.connect(self._show_mini_test_dialog)
        self.mini_test_btn.setVisible(False)  # Hidden by default, shown in debug mode
        load_preview_layout.addWidget(self.mini_test_btn)

        self.install_webengine_btn = QPushButton("Install PyQt6-WebEngine")
        self.install_webengine_btn.clicked.connect(self._show_webengine_installer)
        load_preview_layout.addWidget(self.install_webengine_btn)

        # Check if WebEngine is available and hide the install button if it is
        try:
            # PyQt6 imports (QGIS 4.x requires Qt6)
            self.install_webengine_btn.setVisible(False)
        except ImportError:
            self.install_webengine_btn.setVisible(True)
        ...
        self.show_log_dialog_checkbox.setToolTip(
            "When checked, upload log dialog will appear during upload. "
            "When unchecked, logging is disabled for faster uploads.")
        ...
        self.load_progress_bar.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def _setup_layout_and_signals(self):
        ...
        # Debug mode label (hidden by default)
        self.debug_mode_label = QLabel("Debug Mode On")
        self.debug_mode_label.setStyleSheet("color: blue; font-weight: bold; padding: 4px; background-color: #e6f2ff;")
        self.debug_mode_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.debug_mode_label.setVisible(False)
        main_layout.addWidget(self.debug_mode_label)
    ...
    def __init__(self, parent=None, iface=None):
        ...
        self.iface = iface
        self.plugin_dir = Path(_CURRENT_DIR)

        # Create a .pth file to handle namespace packages correctly
        try:
            pth_path = self.plugin_dir / 'geoserverconnector.pth'
            with open(pth_path, 'w') as f:
                f.write(f"import site; site.addsitedir(r'{self.plugin_dir}')")
        except Exception as e:
            self.log_message(f'Could not create .pth file: {e}', level=Qgis.Warning)
        ...
        # Initialize debug mode (must be after UI setup)
    ...
    def _setup_debug_mode(self):
        """Setup the debug mode shortcut and apply initial state."""
        # Create keyboard shortcut for toggling debug mode
        shortcut = QShortcut(QKeySequence("Ctrl+Alt+Shift+D"), self)
        shortcut.activated.connect(self._toggle_debug_mode)

        # Load saved debug mode state (default to False/OFF)
        settings = QgsSettings()
        is_debug_mode = settings.value("geoserver_connector/debug_mode", False, type=bool)

        # Apply the initial debug mode state
        self._apply_debug_mode(is_debug_mode)

    def _toggle_debug_mode(self):
        """Toggle the debug mode on/off."""
        settings = QgsSettings()
        current_state = settings.value("geoserver_connector/debug_mode", False, type=bool)
        new_state = not current_state

        # Save the new state
        settings.setValue("geoserver_connector/debug_mode", new_state)

        # Apply the new state
        self._apply_debug_mode(new_state)

        # Also update the preview dialog if it's open
        if self.preview_dialog is not None and self.preview_dialog.isVisible():
            if hasattr(self.preview_dialog, 'set_debug_mode'):
                self.preview_dialog.set_debug_mode(new_state)

    def _apply_debug_mode(self, is_enabled):
        """Apply the debug mode state to UI elements."""
        # Show/hide debug mode label
        if hasattr(self, 'debug_mode_label'):
            self.debug_mode_label.setVisible(is_enabled)

        # Show/hide debug buttons
        if hasattr(self, 'mini_test_btn'):
            self.mini_test_btn.setVisible(is_enabled)
        if hasattr(self, 'test_paths_btn'):
            self.test_paths_btn.setVisible(is_enabled)
    ...
    def load_settings_on_startup(self):
        ...
        try:
            # Use self.plugin_dir which is already set correctly in __init__
            settings_file = os.path.join(str(self.plugin_dir), "geoserver_settings.ini")
            print(f"DEBUG: load_settings_on_startup - Looking for settings file at: {settings_file}")

            credentials_found = False

            if os.path.exists(settings_file):
                print("DEBUG: Settings file exists, reading...")
                config = configparser.ConfigParser()
                read_result = config.read(settings_file)
                print(f"DEBUG: ConfigParser read result: {read_result}")
                print(f"DEBUG: Sections found: {config.sections()}")

                # Load settings
                url_val = config.get("GeoServer", "url", fallback="").strip()
                username_val = config.get("GeoServer", "username", fallback="").strip()
                password_val = config.get("GeoServer", "password", fallback="").strip()
                print(f"DEBUG: Loaded values - URL: '{url_val}', Username: '{username_val}', Password: '***'")

                # Check if we have valid credentials (at least URL and username)
                if url_val and username_val:
                    credentials_found = True
                    self.url_input.setText(url_val)
                    self.username_input.setText(username_val)
                    self.password_input.setText(password_val)
                    print("DEBUG: Values set to input fields")

                    # Load UI preferences
                    show_log = config.get("UI", "show_log_dialog", fallback="false").lower() == "true"
                    self.show_log_dialog_checkbox.setChecked(show_log)

                    # Automatically retrieve GeoServer info
                    self.retrieve_geoserver_info()
                else:
                    print("DEBUG: Settings file exists but credentials are empty")
            else:
                print(f"DEBUG: Settings file NOT found at: {settings_file}")

            # If no valid credentials found, prompt user to enter them
            if not credentials_found:
                self._show_first_time_setup_message()
    ...
    def ensure_config_files(self):
        """Ensure local configuration files exist; create empty templates on first load."""
        try:
            plugin_dir = str(self.plugin_dir)
            config_files = {
                "geoserver_settings.ini": (
                    "# GeoServer Connection Settings\n"
                    "# This file stores GeoServer connection credentials for the Q2G QGIS Plugin\n\n"
                    "[GeoServer]\n"
                    "url = \n"
                    "username = \n"
                    "password = \n\n"
                    "[UI]\n"
                    "show_log_dialog = true\n"
                ),
                "postgis.ini": (
                    "# PostGIS Database Credentials\n"
                    "# This file stores database connection credentials for the Q2G QGIS Plugin\n"
                    "# Each section represents a unique database connection\n\n"
                ),
                "auth_config.ini": (
                ...
                if not os.path.exists(config_path):
                    try:
                        with open(config_path, "w") as f:
                            f.write(template)
                        self.log_message(f"✓ Created new {filename}")
                    except Exception as e:
                        self.log_message(
                            f"✗ Failed to create {filename}: {str(e)}",
                            level=Qgis.Warning
                        )
        except Exception as e:
            import traceback
            print(f"DEBUG: Failed to ensure config files: {str(e)}")
            traceback.print_exc()

    def _show_first_time_setup_message(self):
        """Show a message prompting the user to enter GeoServer credentials."""
        from qgis.PyQt.QtWidgets import QMessageBox
        msg = QMessageBox(self)
        msg.setWindowTitle("Welcome to Q2G")
        msg.setIcon(QMessageBox.Icon.Information)
        msg.setText("No GeoServer credentials found.")
        msg.setInformativeText(
            "Please enter your GeoServer connection details:\n\n"
            "1. Enter your GeoServer URL (e.g., http://localhost:8080/geoserver)\n"
            "2. Enter your username and password\n"
            "3. Click 'Save Settings to INI File' to save for future use\n\n"
            "Your credentials will be saved locally for automatic loading next time."
        )
        msg.setStandardButtons(QMessageBox.StandardButton.Ok)
        msg.exec()

    def show_debug_paths_dialog(self):
        """Show a dialog with diagnostic path information."""
        import platform
        import glob
        from qgis.PyQt.QtWidgets import QDialog, QVBoxLayout, QTableWidget, QTableWidgetItem, QHeaderView, QPushButton

        # --- 1. Perform all checks and gather all data first ---
        is_webengine_importable = False
        webengine_location = "Not found"
        import_error_message = ""
        try:
            # Try PyQt6 first (QGIS 3.28+)
            is_webengine_importable = True
            import PyQt6.QtWebEngineWidgets as qwe
            if hasattr(qwe, '__file__') and qwe.__file__:
                webengine_location = qwe.__file__
            else:
                webengine_location = "Loaded (location not available via __file__)"
        except ImportError as e:
            import_error_message = str(e)

        # --- 2. Build the list of diagnostic items based on the check ---
        diagnostics = []
        system = platform.system()
        qgis_exe = sys.executable
        qgis_dir = os.path.dirname(os.path.dirname(qgis_exe))
        str(self.plugin_dir)

        # Basic QGIS info
        diagnostics.append(("QGIS Installation Folder", os.path.dirname(qgis_dir)))
        diagnostics.append(("QGIS Executable (sys.executable)", qgis_exe))

        # Python executable path
        found_python = None
        if system == 'Windows':
            python_patterns = [os.path.join(qgis_dir, 'apps', 'Python3*', 'python.exe'),
                               os.path.join(qgis_dir, 'apps', 'Python*', 'python.exe')]
            ...
            py_name = 'python3'
            python_patterns = ['/usr/bin/python3', os.path.join(qgis_dir, 'bin', py_name)]
            if system == 'Darwin':
                python_patterns.insert(0, '/Applications/QGIS.app/Contents/MacOS/bin/python3')
            for pattern in python_patterns:
                if os.path.exists(pattern):
                    found_python = pattern
                    break
        diagnostics.append(("Python Executable Path", found_python if found_python else "NOT FOUND"))

        # WebEngine Status
        diagnostics.append(("PyQt6-WebEngine importable?",
                           "YES - Import successful" if is_webengine_importable else f"NO - {import_error_message}"))
        diagnostics.append(("PyQt6-WebEngine location", webengine_location))

        # Summary Status
        status_summary = ""
        if is_webengine_importable:
            if ('site-packages' in webengine_location and 'QGIS' not in webengine_location
                    and 'geoserverconnector' not in webengine_location):
                status_summary = (
                    "OK - Using existing installation from a user Python environment "
                    "(e.g., AppData). No action needed."
                )
            elif 'geoserverconnector' in webengine_location:
                status_summary = "OK - Using local installation in the plugin folder."
            else:
                status_summary = "OK - Using existing installation from QGIS directory."
        else:
            status_summary = "NOT FOUND - Please use the 'Install' button or follow the manual command below."
        diagnostics.append(("Status Summary", status_summary))

        # Installation commands (only if needed)
        if not is_webengine_importable:
            import site
            user_site = site.getusersitepackages()
            if found_python:
                pip_command = (
                    f'\"{found_python}\" -m pip install --upgrade --user '
                    f'PyQt6-WebEngine==6.8.0 --timeout 120'
                )
                diagnostics.append(("Full pip install command", pip_command))
                diagnostics.append(("Target Installation Path", user_site))
                if system == 'Windows':
                    manual_cmd = (
                        f'cd /d \"{os.path.dirname(found_python)}\" && python.exe -m pip install '
                        f'--upgrade --user PyQt6-WebEngine==6.8.0 --timeout 120'
                    )
                else:
                    manual_cmd = pip_command  # On Linux/macOS, the command is the same
                diagnostics.append(("Full installation command line (for terminal)", manual_cmd))
            else:
                diagnostics.append(("Installation Command", "Could not determine Python executable path."))

        # --- 3. Create and show the dialog ---
        dialog = QDialog(self)
        dialog.setWindowTitle("Debug Paths - Diagnostic Info")
        ...
        table.setHorizontalHeaderLabels(["Description", "Value"])
        table.setRowCount(len(diagnostics))
        table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)

        for row, (desc, value) in enumerate(diagnostics):
            item_desc = QTableWidgetItem(desc)
            item_value = QTableWidgetItem(str(value))
            item_value.setToolTip(str(value))  # Add tooltip for the full path
    ...
    def save_settings_to_file(self):
        ...
        default_path = os.path.join(plugin_dir_str, "geoserver_settings.ini")
        print(f"DEBUG: save_settings_to_file - plugin_dir: {plugin_dir_str}")
        print(f"DEBUG: save_settings_to_file - default_path: {default_path}")
    ...
    def check_postgis_credentials_setup(self):
        ...
        if not os.path.exists(self.postgis_credentials.config_path):
            ...
            try:
                ...
                self.log_message("✓ Created new postgis.ini file")
            except Exception as e:
        ...
        try:
            ...
            if not connections:
                self.log_message(
                    "ℹ No PostGIS credentials found. You may need to set up "
                    "PostGIS credentials for database connections.")
            else:
    ...
    def retrieve_geoserver_info(self):
        ...
        try:
            ...
            response = requests.get(f"{url}/rest/workspaces.xml", auth=(username, password),
    ...
    def load_stores(self):
        ...
        # Fetch datastores
        try:
            response = requests.get(f"{url}/rest/workspaces/{workspace}/datastores.json", auth=(username, password), timeout=30)
        ...
        try:
            response = requests.get(
                f"{url}/rest/workspaces/{workspace}/coveragestores.json", auth=(username, password), timeout=30)
            if response.status_code == 200:
    ...
    def _get_user_friendly_error_message(self, error_str):
        ...
        # Connection timeout
            ...
            return (
                ...
                "GeoServer is taking too long to respond.\n\n"
                "Please check:\n"
                "• Is GeoServer running and responsive?\n"
    ...
    def load_workspace_layers(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                QMessageBox.warning(
                    self, "Error",
                    f"Failed to fetch workspace layers. Status: {response.status_code}\n{response.text}")
        except requests.exceptions.RequestException as e:
            error_msg = self._get_user_friendly_error_message(str(e))
            QMessageBox.critical(self, "Connection Error", error_msg)
        except Exception as e:
    ...
    def filter_workspace_layers(self):
        ...
        """
        _wf = _load_local_module("wildcard_filter")
        WildcardFilter = _wf.WildcardFilter
    ...
    def _update_datastores_for_filtered_layers(self, filtered_layers):
        ...
        try:
            ...
            for layer_name in filtered_layers:
                ...
                layer_response = requests.get(layer_url, auth=(username, password), timeout=30)
    ...
    def _on_upload_sld_clicked(self, layer):
        ...
        if show_log_dialog:
            self.log_tracker.show_live_log_window(self, "Upload SLD Progress")
    ...
    def _upload_layer_sld_to_geoserver(self, layer, layer_name, workspace, url, username, password):
        ...
        try:
            # Reset ELSE choice for single layer upload (each manual upload is a fresh session)
            self.sld_style_uploader.reset_else_choice()

            self.log_message(f"Exporting and uploading SLD for layer '{layer_name}' to GeoServer...")

            # Use the centralized upload method so rule-based Path B and cased-lines apply
            success = self.sld_style_uploader.upload_sld_style(
                layer, layer_name, workspace, url, username, password,
                cased_lines=self.cased_lines_checkbox.isChecked()
            )

            if success is None:
                self._show_sld_status("SLD upload cancelled")
            elif not success:
    ...
    def _show_sld_status(self, message):
        ...
        for i, opacity in enumerate(fade_steps):
            QTimer.singleShot(
                5000 + (i * 300),
                lambda op=opacity, color=base_color: self.sld_status_label.setStyleSheet(
                    f"color: rgba({color}, {op}); font-size: 10pt;"))
    ...
    def _collect_checked_layers_recursive(self, item, selected_layers):
        ...
        is_group = item.data(0, Qt.ItemDataRole.UserRole + 3)  # True if marked as group
    ...
    def _detect_file_type_and_validate(self, layer):
        ...
        try:
            ...
            # Determine file type
            is_shapefile = source_path.lower().endswith('.shp') or (
                os.path.isdir(source_path) and '|layername=' in layer.source())
            ...
            self.log_message(
                f"DEBUG: File type detection - Shapefile: {is_shapefile}, "
                f"GeoPackage: {is_geopackage}, GeoTIFF: {is_geotiff}, "
                f"GeoJSON: {is_geojson}, KML: {is_kml}, CSV: {is_csv}")
    ...
    def _create_import_job(self, workspace, url, username, password):
        ...
        try:
            ...
            if import_id is None:
                self.log_message("❌ No import ID returned", level=Qgis.Critical)
                return None
            ...
            with open(source_path, 'rb') as file_data:
                ...
                if layer:
                    ...
                    if '|layername=' in layer_source:
                        ...
                        self.log_message(
                            "DEBUG: No layername parameter found in source, uploading without layer specification")
    ...
    def _upload_shapefile(self, layer, source_path, layer_name, tasks_url, username, password):
        ...
        """
        # Get shapefile info
        folder_path, shapefile_name, _ = self._get_shapefile_info(layer) if os.path.isdir(source_path) else (
            os.path.dirname(source_path), os.path.splitext(os.path.basename(source_path))[0], None)
        ...
        try:
            ...
            with open(temp_zip_path, 'rb') as f:
                return requests.post(tasks_url, auth=(username, password), files={'file': (f"{layer_name}.zip", f)}, timeout=30)
        finally:
    ...
    def _upload_geotiff_file(self, source_path, tasks_url, username, password):
        ...
        with open(source_path, 'rb') as f:
            return requests.post(tasks_url, files={'file': (
                os.path.basename(source_path), f)}, auth=(username, password), timeout=30)
    ...
    def _execute_import_job(self, import_id, url, username, password):
        ...
        try:
            ...
            execute_resp = requests.post(execute_url, auth=(username, password), timeout=30)
            ...
            self.log_message("✓ Import executed")
            return True
    ...
    def _upload_layer_importer(self, layer, layer_name, workspace, url, username, password):
        ...
        try:
            ...
            self.log_message("✓ File uploaded")
    ...
    def _upload_unsupported_as_geopackage(self, layer, layer_name, workspace, url, username, password):
        ...
        try:
            ...
            if not isinstance(layer, QgsVectorLayer):
                self.log_message(
                    f"❌ Unsupported format for layer '{layer.name()}' - "
                    f"only vector layers can be converted", level=Qgis.Critical)
                return False
            ...
            self.log_message("✓ GeoPackage created successfully")
            ...
            if not temp_layer.isValid():
                self.log_message("❌ Failed to load converted GeoPackage", level=Qgis.Critical)
                return False
            ...
            self.log_message("📤 Uploading converted GeoPackage to GeoServer")
            success = self._register_geopackage_datastore_and_publish(
                temp_layer, layer_name, workspace, url, username, password)
    ...
    def _upload_layer_importer_main18(self, layer, workspace, url, username, password):
        ...
        try:
            source_path = layer.source().split('|')[0].split('?')[0]
            self._sanitize_layer_name(layer.name())
            self.log_message(
                f"DEBUG IMPORTER: Starting upload for layer '{layer.name()}' "
                f"using Importer API to workspace '{workspace}'")
            ...
            if source_path.lower().endswith('.gpkg') and '|layername=' in layer_source:
                ...
                with open(source_path, 'rb') as file_data:
                    ...
                    upload_response = requests.post(tasks_url, auth=(username, password), files=files, data=data, timeout=30)
            else:
                # Standard file upload for other formats
                with open(source_path, 'rb') as file_data:
                    files = {'file': (os.path.basename(source_path), file_data)}
                    upload_response = requests.post(tasks_url, auth=(username, password), files=files, timeout=30)
            ...
            execute_response = requests.post(execute_url, auth=(username, password), timeout=30)
            ...
            try:
                ...
                import_status_response = requests.get(f"{url}/rest/imports/{import_id}", auth=(username, password), timeout=30)
                ...
                tasks_response = requests.get(tasks_url, auth=(username, password), timeout=30)
                    ...
                    for task in tasks_data.get('tasks', []):
                        ...
                        if task_state == 'READY':
                            ...
                            if task_id:
                                ...
                                task_response = requests.post(task_execute_url, auth=(username, password), timeout=30)
                            ...
                            if task_id:
                                ...
                                if layer_crs.isValid():
                                    ...
                                    crs_response = requests.put(
                                        task_update_url, auth=(username, password),
                                        json=crs_payload,
                                        headers={"Content-Type": "application/json"}, timeout=30)
                                    ...
                                    if crs_response.status_code in [200, 201]:
                                        ...
                                        if task_response.status_code in [200, 201, 202]:
                                            self.log_message(
                                                f"Successfully executed task {task_id} with CRS {crs_code}")
                                            layers_published += 1
                                        else:
                                            self.log_message(
                                                f"Failed to execute task {task_id} after CRS "
                                                f"update: {task_response.text}")
                                    else:
                                        self.log_message(
                                            f"Failed to update CRS for task {task_id}: {crs_response.text}")
                                else:
    ...
    def _cleanup_all_duplicate_stores_for_layer(self, layer_name, workspace, url, username, password):
        ...
        Args:
            ...
            password: GeoServer password
        """
        return self.all_stores_cleaner.cleanup_all_duplicate_stores_for_layer(
            layer_name, workspace, url, username, password)
    ...
    def _cleanup_duplicate_datastores(self, layer_name, workspace, url, username, password):
        ...
        Args:
            ...
            password: GeoServer password
        """
        return self.duplicate_datastores_cleaner.cleanup_duplicate_datastores(
            layer_name, workspace, url, username, password)
    ...
    def _geopackage_layer_exists_in_datastore(
            self, layer_name, gpkg_datastore_name, workspace, url, username, password):
        """
        ...
        """
        return self.geopackage_layer_checker.geopackage_layer_exists_in_datastore(
            layer_name, gpkg_datastore_name, workspace, url, username, password)
    ...
    def _upload_layer_native(self, layer, layer_name, workspace, url, username, password, provider_info):
        ...
        if upload_method == 'gpkg_native':
            ...
            return self._register_geopackage_datastore_and_publish(
                layer, layer_name, workspace, url, username, password)
            ...
            self.log_message(
                f"Unsupported native upload method '{upload_method}', falling back to GeoPackage conversion")
    ...
    def _register_geopackage_datastore_and_publish(self, layer, layer_name, workspace, url, username, password):
        ...
        Returns:
            bool: True if successful, False otherwise
        """
        return self.geopackage_publisher.register_geopackage_datastore_and_publish(
            layer, layer_name, workspace, url, username, password)
        ...
        try:
            ...
            for item in selected_items:
                ...
                if not layer_id:
                    ...
                    is_checked = item.checkState(0) == Qt.CheckState.Checked.value
    ...
    def _update_select_all_qgis_layers_checkbox(self):
        ...
        try:
            ...
            for i in range(root.childCount()):
                item = root.child(i)
                if item.data(0, Qt.ItemDataRole.UserRole):  # Has layer ID, so it's a layer not a group
                    total_selectable += 1
                    if item.checkState(0) == Qt.CheckState.Checked.value:
                        checked_count += 1
                else:
                    # It's a group, check its children
                    for j in range(item.childCount()):
                        child = item.child(j)
                        if child.data(0, Qt.ItemDataRole.UserRole):  # Has layer ID
                            total_selectable += 1
                            if child.checkState(0) == Qt.CheckState.Checked.value:
                                checked_count += 1
    ...
    def _on_qgis_tree_item_changed(self, item, column):
        ...
        try:
            ...
            if layer and isinstance(layer, (QgsVectorLayer, QgsRasterLayer)):
                ...
                is_visible = new_state == Qt.CheckState.Checked.value
    ...
    def upload_sld_style(self, layer, layer_name, base_url, username, password, workspace):
        ...
        Args:
            ...
            workspace: Target workspace name
        """
        return self.sld_style_uploader.upload_sld_style(
            layer, layer_name, workspace, base_url, username, password,
            cased_lines=self.cased_lines_checkbox.isChecked()
        )
    ...
    def _show_workspace_context_menu(self, position):
        ...
        context_menu.exec(self.workspaces_list.mapToGlobal(position))
    ...
    def _on_rename_workspace_context(self):
        ...
        try:
            ...
            response = requests.put(
                ...
                json=workspace_data, timeout=30)

            # Handle response
            if response.status_code == 200:
                QMessageBox.information(
                    self, "Success", f"Workspace '{old_name}' renamed to '{new_name}' successfully.")
                ...
                self.log_message(
                    f"Failed to rename workspace {old_name}: Status {response.status_code} - {response.text}")
    ...
    def _on_delete_workspace_content_context(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Success", f"All content in workspace '{workspace_name}' has been deleted successfully.")
            ...
            for datastore in datastores:
                ...
                if datastore_name:
                    ...
                    if delete_response.status_code in [200, 204]:
                        ...
                        self.log_message(
                            f"⚠ Failed to delete datastore '{datastore_name}': {delete_response.status_code}")
            ...
            for coveragestore in coveragestores:
                ...
                if coveragestore_name:
                    ...
                    if delete_response.status_code in [200, 204]:
                        ...
                        self.log_message(
                            f"⚠ Failed to delete coverage store '{coveragestore_name}': {delete_response.status_code}")
    ...
    def select_all_datastores(self, state):
        ...
        self.datastores_list.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
    ...
    def show_documentation(self):
        ...
        try:
            ...
            # Load documentation module from local file
                        ...
                        if resource_class == 'featureType':
                            ...
                            if ft_url:
                                ...
                                if ft_response.status_code == 200:
                                    ...
                                    details_text += "  Type: Vector (FeatureType)\n"
                            ...
                            if cov_url:
                                ...
                                if cov_response.status_code == 200:
                                    ...
                                    details_text += "  Type: Raster (Coverage)\n"
    ...
    def _execute_copy_move_operation(self, layer_names, source_workspace,
                                     target_workspace, operation_mode, conflict_strategy):
        """Execute copy/move operation directly without any dialog."""
        try:
            print("DEBUG: _execute_copy_move_operation called")
            print(
                f"DEBUG: Layers: {layer_names}, Source: {source_workspace}, "
                f"Target: {target_workspace}, Mode: {operation_mode}, "
    ...
    def _open_copy_move_dialog_with_layers(self, layer_names, source_workspace, target_workspace, operation_mode):
        ...
        try:
            print("DEBUG: _open_copy_move_dialog_with_layers called")
            print(
                f"DEBUG: Layers: {layer_names}, Source: {source_workspace}, "
                f"Target: {target_workspace}, Mode: {operation_mode}")

            _cmld = _load_local_module("copy_move_layers_dialog")
            CopyMoveLayersDialog = _cmld.CopyMoveLayersDialog
    ...
    def _show_layers_context_menu(self, position):
        ...
        try:
            ...
            action = menu.exec(self.workspace_layers_list.mapToGlobal(position))
    ...
    def _show_preview_dialog(self):
        ...
        try:
            ...
            if result:
                ...
                # Apply current debug mode state to the preview dialog
                settings = QgsSettings()
                is_debug_mode = settings.value("geoserver_connector/debug_mode", False, type=bool)
                if hasattr(self.preview_dialog, 'set_debug_mode'):
                    self.preview_dialog.set_debug_mode(is_debug_mode)
            else:
            ...
            if 'QtWebEngineWidgets' in str(e) or 'ModuleNotFoundError' in str(type(e).__name__):
                # Show the WebEngine installer dialog
                self.log_message(
                    "⚠️ Preview not available: QtWebEngineWidgets missing - showing installer", level=Qgis.Warning)
                self._show_webengine_installer()
            else:
    ...
    def _show_webengine_installer(self):
        """Open the PyQt6-WebEngine installer dialog manually."""
        try:
            InstallerDialog = dynamic_import("pyqtwebengine_installer", "PyQtWebEngineInstallerDialog")
            dialog = InstallerDialog(self)
            dialog.exec()
        except Exception as e:
            error_msg = f"Could not open the installer dialog: {e}"
            self.log_message(f"❌ ERROR: {error_msg}", level=Qgis.Critical)
            QMessageBox.critical(self, "Installer Error", error_msg)

    def _show_mini_test_dialog(self):
        """Open the minimal WebEngine test dialog for diagnostics."""
        try:
            self.log_message("🧪 Opening Mini WebEngine Test dialog...")
            open_mini_dialog = dynamic_import("mini", "open_mini_dialog")
            self.mini_dialog = open_mini_dialog(self)
            self.log_message("✅ Mini WebEngine Test dialog opened")
        except Exception as e:
            import traceback
            error_msg = f"Could not open mini test dialog: {e}"
            self.log_message(f"❌ ERROR: {error_msg}", level=Qgis.Critical)
            traceback.print_exc()
            QMessageBox.critical(self, "Mini Test Error", error_msg)
...
if __name__ == "__main__":
    ...
    dialog = QGISGeoServerLayerLoader(_iface.mainWindow(), _iface)
    dialog.show()
