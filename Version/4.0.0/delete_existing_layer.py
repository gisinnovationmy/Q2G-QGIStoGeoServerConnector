...
class ExistingLayerDeleter:
        ...
        if layers_response.status_code == 200:
            ...
            if 'layers' in layers_data and 'layer' in layers_data['layers']:
                for layer_info in layers_data['layers']['layer']:
                    ...
                    if (layer_actual_name == layer_name
                        or layer_actual_name.startswith(layer_name)
                            or layer_name in layer_actual_name):
    ...
    def _delete_single_layer(self, actual_layer_name, workspace, url, username, password):
        ...
        self._delete_associated_store(actual_layer_name, workspace, url, username, password)
    ...
    def _delete_layer(self, actual_layer_name, workspace, url, username, password):
        ...
        response = requests.delete(layer_url, auth=(username, password), timeout=30)
        ...
        if response.status_code in [200, 204]:
            ...
            self.main.log_message(
                f"Warning: Could not delete layer '{actual_layer_name}': {response.status_code}", level=Qgis.Warning)
            return False
    ...
    def _delete_featuretypes_from_datastores(self, actual_layer_name, workspace, url, username, password):
        ...
        try:
            ...
            ds_list_resp = requests.get(datastores_url, auth=(username, password), timeout=30)
            if ds_list_resp.status_code == 200:
                ...
                for ds in datastores:
                    ...
                    ft_check = requests.get(ft_url, auth=(username, password), timeout=30)
                    if ft_check.status_code == 200:
                        # Featuretype exists in this datastore, delete it
                        ft_delete = requests.delete(ft_url, auth=(username, password), params={'recurse': 'true'}, timeout=30)
                        if ft_delete.status_code in [200, 204]:
                            self.main.log_message(
                                f"🗑️ Deleted featuretype '{actual_layer_name}' from datastore '{ds_name}'")
    ...
    def _delete_store_by_layer_info(self, actual_layer_name, workspace, url, username, password):
        ...
        try:
            ...
            layer_info_response = requests.get(layer_info_url, auth=(username, password), timeout=30)
                ...
                if 'resource' in layer_info:
                    ...
                    if 'href' in resource_info:
                        ...
                        if '/datastores/' in href:
                            return self._delete_datastore_from_href(
                                href, actual_layer_name, workspace, url, username, password)

                        # For raster layers: .../coveragestores/ACTUAL_STORE_NAME/coverages/...
                        elif '/coveragestores/' in href:
                            return self._delete_coveragestore_from_href(
                                href, actual_layer_name, workspace, url, username, password)
        except Exception as e:
    ...
    def _delete_datastore_from_href(self, href, actual_layer_name, workspace, url, username, password):
        ...
        ds_response = requests.delete(datastore_url, auth=(username, password), params={'recurse': 'true'}, timeout=30)
    ...
    def _delete_coveragestore_from_href(self, href, actual_layer_name, workspace, url, username, password):
        ...
        cs_response = requests.delete(coveragestore_url, auth=(username, password), params={'recurse': 'true'}, timeout=30)
