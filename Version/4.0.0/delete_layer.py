...
class LayerDeletionManager:
    ...
    def _confirm_deletion(self, workspace_name, layer_names):
        ...
        reply = QMessageBox.question(
            self.main,
            'Confirm Deletion',
            f"Are you sure you want to delete {len(layer_names)} layers and their stores from "
            f"'{workspace_name}'?\n\n" + "\n".join(layer_names),
    ...
    def _delete_single_layer(self, layer_name, workspace_name, url, username, password):
        ...
        try:
            ...
            self.main.log_message(
                f"Determined store for '{layer_name}' is '{store_name}' (type: {store_type or 'unknown'})")
    ...
    def _find_layer_store(self, layer_name, workspace_name, url, username, password):
        ...
        response = requests.get(layer_details_url, auth=(username, password), timeout=30)
        ...
        if response.status_code == 200:
            ...
            self.main.log_message(
                f"Could not get layer details for '{layer_name}'. Status: {response.status_code}", level=Qgis.Warning)
    ...
    def _delete_store(self, layer_name, store_name, store_type, workspace_name, url, username, password):
        ...
        if store_type is None:
            self.main.log_message("Store type is unknown, trying datastore first.")
            delete_url = f"{url}/rest/workspaces/{workspace_name}/datastores/{store_name}?recurse=true"
            response = requests.delete(delete_url, auth=(username, password), timeout=30)
        ...
        if response.status_code == 200:
            ...
            self.main.log_message(
                f"Failed to delete store for layer '{layer_name}': "
