...
class AllDuplicateStoresForLayerCleaner:
    ...
    def cleanup_all_duplicate_stores_for_layer(self, layer_name, workspace, url, username, password):
        ...
        try:
            ...
            self.main.log_message(
                f"DEBUG: Cleaning up all duplicate stores for '{layer_name}' (sanitized: {sanitized_layer})")
    ...
    def _delete_workspace_layer(self, sanitized_layer, workspace, url, username, password):
        ...
        layer_check = requests.get(layer_url, auth=(username, password), timeout=30)
        ...
        if layer_check.status_code == 200:
            ...
            delete_layer_resp = requests.delete(delete_layer_url, auth=(username, password), params={'recurse': 'true'}, timeout=30)
    ...
    def _cleanup_datastores(self, sanitized_layer, workspace, url, username, password):
        ...
        ds_response = requests.get(datastores_url, auth=(username, password), timeout=30)
    ...
    def _cleanup_coverage_stores(self, sanitized_layer, workspace, url, username, password):
        ...
        cs_response = requests.get(coveragestores_url, auth=(username, password), timeout=30)
    ...
    def _delete_datastore_with_retry(self, store_name, workspace, url, username, password):
        ...
        delete_response = requests.delete(delete_url, auth=(username, password), params={'recurse': 'true'}, timeout=30)
        ...
        if delete_response.status_code in [200, 204]:
            ...
            if delete_response.status_code in [200, 204]:
                ...
                self.main.log_message(
                    f"⚠ Failed to delete datastore {store_name} (retry): {delete_response.status_code}")
        else:
    ...
    def _delete_coverage_store_with_retry(self, store_name, workspace, url, username, password):
        ...
        if delete_response.status_code in [200, 204]:
            ...
            if delete_response.status_code in [200, 204]:
                ...
                self.main.log_message(
                    f"⚠ Failed to delete coverage store {store_name} (retry): {delete_response.status_code}")
        else:
