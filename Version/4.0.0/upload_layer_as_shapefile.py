    ...
    try:
        ...
        try:
            ...
            if hasattr(QgsVectorFileWriter, 'NoError') and err_code != QgsVectorFileWriter.NoError:
                log_message(
                    f"Failed exporting Shapefile for '{layer_name}'. Error code: {err_code}", level=Qgis.Critical)
        ...
        with open(zip_path, 'rb') as fh:
            ...
            resp = requests.put(upload_url, data=fh, headers=headers, auth=(username, password), timeout=30)
