...
class TaskVerificationPublisher:
        ...
        try:
            ...
            for idx, t in enumerate(all_tasks):
                ...
                self.main.log_message(
                    f"DEBUG: Task {idx}: ID={t.get('id')}, State={t.get('state')}, Layer={published_name}")
            ...
            return self._process_final_result(layers_published, layer, workspace, url,
                                              username, password, published_layer_names)
            ...
            if crs_code:
                ...
                if crs_response.status_code in [200, 201]:
                    ...
                    if task_response.status_code in [200, 201, 202]:
                        ...
                        self.main.log_message(
                            f"❌ Failed to execute task {task_id} after CRS update: "
                            f"{task_response.text}", level=Qgis.Critical)
                else:
                    self.main.log_message(
                        f"❌ Failed to update CRS for task {task_id}: "
                        f"Status {crs_response.status_code} - {crs_response.text}",
                        level=Qgis.Critical)
        return 0
    ...
    def _process_final_result(self, layers_published, layer, workspace, url,
                              username, password, published_layer_names=None):
        """
        ...
        if layers_published > 0:
            ...
            self.main.log_message(
                f"⚠️ No layers were published from '{layer.name()}' (may be unsupported format)", level=Qgis.Warning)
            return False
    ...
    def _rename_geotiff_layer_if_needed(self, layer, workspace, url, username, password, published_layer_names=None):
        ...
        try:
            ...
            if not is_geotiff:
                self.main.log_message("DEBUG: Not a GeoTIFF file, skipping rename")
                return
            ...
            if actual_name != original_name:
                ...
                success = self._rename_coverage_in_datastore(
                    actual_name, original_name, workspace, url, username, password)
                ...
                if success:
                    ...
                    self.main.log_message("⚠️ Failed to rename layer", level=Qgis.Warning)
            else:
    ...
    def _find_actual_published_layer(self, original_name, workspace, url, username, password):
        ...
        try:
            ...
            layers_response = requests.get(layers_url, auth=(username, password), timeout=30)
    ...
    def _find_published_coverage_name(self, expected_name, workspace, url, username, password):
        ...
        try:
            ...
            if layer_response.status_code == 200:
                self.main.log_message(
                    f"DEBUG: Layer '{expected_name}' already exists at workspace level - no rename needed")
            ...
            stores_response = requests.get(stores_url, auth=(username, password), timeout=30)
            ...
            for store in stores:
                ...
                coverages_response = requests.get(coverages_url, auth=(username, password), timeout=30)
                        ...
                        if isinstance(coverage, dict):
                            ...
                            if coverage_name:
                                ...
                                if coverage_name != expected_name:
                                    self.main.log_message(
                                        f"DEBUG: Coverage '{coverage_name}' differs from "
                                        f"expected '{expected_name}' - will rename")
                                    return coverage_name
                                else:
                                    self.main.log_message(
                                        f"DEBUG: Coverage '{coverage_name}' matches expected name - no rename needed")
                                    return None
                        else:
                            self.main.log_message(f"DEBUG: Invalid coverage format: {type(coverage)} - {coverage}")

            self.main.log_message("DEBUG: No coverages found in any store")
            return None
                ...
                if coverages_response.status_code == 200:
                    ...
                    for coverage in coverage_list:
                        if isinstance(coverage, dict) and coverage.get('name') == old_name:
                            ...
                            rename_url = (
                                f"{url}/rest/workspaces/{workspace}/coveragestores/"
                                f"{store_name}/coverages/{old_name}")
                            rename_payload = {
                                "coverage": {
                                    "name": new_name
                                }
                            ...
                            if rename_response.status_code in [200, 201]:
                                ...
                                self.main.log_message(
                                    f"DEBUG: Renaming layer at workspace level: {old_name} → {new_name}")
                                ...
                                if layer_rename_response.status_code in [200, 201]:
                                    self.main.log_message(
                                        f"DEBUG: Layer rename response: {layer_rename_response.status_code}")
                                    return True
                                else:
                                    self.main.log_message(
                                        f"DEBUG: Layer rename failed: "
                                        f"{layer_rename_response.status_code} - "
                                        f"{layer_rename_response.text}")
                                    # Coverage was renamed but layer wasn't - still consider it partial success
                                    return True
                            else:
                                self.main.log_message(
                                    f"DEBUG: Coverage rename failed: "
