...
class DuplicateGlobalStylesCleaner:
    ...
    def cleanup_duplicate_global_styles(self, workspace, url, username, password):
        ...
        try:
            ...
            deleted_count = self._delete_duplicate_global_styles(
                global_styles, workspace_style_names, url, username, password)
    ...
    def _fetch_global_styles(self, url, username, password):
        ...
        response = requests.get(global_styles_url, auth=(username, password), timeout=30)
    ...
    def _fetch_workspace_style_names(self, workspace, url, username, password):
        ...
        ws_response = requests.get(workspace_styles_url, auth=(username, password), timeout=30)
    ...
    def _delete_global_style(self, style_name, url, username, password):
        ...
        delete_response = requests.delete(delete_url, auth=(username, password), params={'purge': 'true'}, timeout=30)
        ...
        if delete_response.status_code in [200, 204]:
            ...
            self.main.log_message(
                f"⚠ Failed to delete duplicate global style {style_name}: {delete_response.status_code}")
            return False
