...
class GeoPackageNativeUploader:
    ...
    def upload_geopackage(self, layer, workspace, url, username, password):
        ...
        try:
            ...
            self.main.log_message("🔄 Native GeoPackage upload starting...")
            ...
            if not self._publish_layer(gpkg_layer_name, sanitized_layer_name,
                                       datastore_name, workspace, url, username, password):
                return False
    ...
    def upload_geopackage_batch(self, layers, workspace, url, username, password):
        ...
        try:
            ...
            self.main.log_message("🔄 Batch GeoPackage upload starting...")
            self.main.log_message(f"   Source: {source_path}")
            self.main.log_message(f"   Datastore: {datastore_name}")
            ...
            for layer in layers:
                ...
                if '|layername=' in layer_source:
                    ...
                    self.main.log_message(
                        f"⚠️ Cannot determine layer name for '{layer.name()}' - skipping", level=Qgis.Warning)
                    all_success = False
                    continue
            ...
            if all_success:
                ...
                self.main.log_message("⚠️ Some layers failed to publish from GeoPackage", level=Qgis.Warning)
    ...
    def _upload_file_to_server(self, source_path, datastore_name, url, username, password):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.main.log_message(
                    f"❌ Failed to upload GeoPackage file: {response.status_code} - {response.text}",
                    level=Qgis.Critical
                )
                return False
    ...
    def _ensure_datastore_exists(self, datastore_name, workspace, url, username, password):
        ...
        try:
            ...
            check_response = requests.get(check_url, auth=(username, password), timeout=30)
            ...
            if check_response.status_code == 200:
                ...
                self.main.log_message(
                    f"❌ Error checking datastore existence: {check_response.status_code}", level=Qgis.Critical)
                return False
    ...
    def _create_new_datastore(self, datastore_name, workspace, url, username, password):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.main.log_message(
                    f"❌ Failed to create datastore: {response.status_code} - {response.text}", level=Qgis.Critical)
                return False
    ...
    def _publish_layer(self, gpkg_layer_name, geoserver_layer_name, datastore_name, workspace, url, username, password):
        ...
        try:
            ...
            check_layer_url = (
                f"{url}/rest/workspaces/{workspace}/datastores/{datastore_name}/"
                f"featuretypes/{geoserver_layer_name}"
            )
            check_response = requests.get(check_layer_url, auth=(username, password), timeout=30)
            ...
            if check_response.status_code == 200:
                ...
                self.main.log_message(
                    f"❌ Error checking layer existence: {check_response.status_code}", level=Qgis.Critical)
                return False
            ...
            if response.status_code in [200, 201]:
                ...
                self.main.log_message(
                    f"❌ Failed to publish layer: {response.status_code} - {response.text}", level=Qgis.Critical)
                return False
