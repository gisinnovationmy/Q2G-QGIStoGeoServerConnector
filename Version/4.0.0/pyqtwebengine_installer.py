...
def get_webengine_package_name():
    """Get the correct WebEngine package name (PyQt6 for QGIS 4.x)."""
...
class InstallThread(QThread):
    ...
    def run(self):
        try:
            ...
            # Check for existing installation (PyQt6 for QGIS 4.x)
            ...
            # sys.executable might point to qgis-ltr-bin.exe or qgis-bin, so we need
            # to find the actual Python executable
            import platform
            import glob
                ...
                if result.returncode == 0:
                    ...
                    if "timed out" in error_msg.lower() or "timeout" in error_msg.lower():
                        if attempt < max_retries:
                            self.progress.emit("⚠️ Network timeout. Retrying in 5 seconds...\n\n")
                            import time
                            time.sleep(5)
                            continue
...
class PyQtWebEngineInstallerDialog(QDialog):
    ...
    def init_ui(self):
        ...
        desc = QLabel(
            ...
            f"Detected: Qt{self.qt_version} - Click 'Install' to automatically "
            f"install it for your QGIS Python environment."
        )
        ...
        self.internet_notice.setStyleSheet(
            "color: #ff6b6b; padding: 10px; background-color: #ffe0e0; border-radius: 5px;")
