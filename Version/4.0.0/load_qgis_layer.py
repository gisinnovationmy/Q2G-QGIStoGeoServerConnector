...
class QGISLayerLoader:
    ...
    def load_qgis_layer(self):
        ...
        if not all([url, username, password, workspace]):
            QMessageBox.warning(self.main, "Input Error",
                                "Please fill in all GeoServer connection details and select a workspace.")
            return
        ...
        if show_log_dialog:
            self.main.log_tracker.show_live_log_window(self.main, "Upload Progress")
    ...
    def _start_upload_thread(self, url, username, password, workspace):
        ...
        # Create a function that captures all parameters
        def upload_func():
            return self._perform_upload(url, username, password, workspace)
    ...
    def _perform_upload(self, url, username, password, workspace):
        ...
        try:
            ...
            # Reset ELSE rule handling choice for this batch
            if hasattr(self.main, 'sld_uploader'):
                self.main.sld_uploader.reset_else_choice()
            ...
            if duplicates_dict:
                ...
                for layer_name, dup_info in duplicates_dict.items():
                    self.main.log_message(
                        f"  Layer '{layer_name}': Keeping {dup_info['selected_format']}, "
                        f"skipping {len(dup_info['duplicates'])} duplicate(s)")
            ...
            for i, layer in enumerate(selected_layers):
                ...
                if self.main.upload_stop_requested:
                    ...
                    for remaining_layer in selected_layers:
                        if id(remaining_layer) not in processed_layers:
                            self.main.log_message(
                                f"⏭️ Skipping layer '{remaining_layer.name()}' - upload stopped by user")
                ...
                self.main.log_message(
                    f"DEBUG: Layer '{layer.name()}' format: {native_format}, upload method: {upload_method}")

                # Get source path for duplicate detection
                source_path = layer.source().split('|')[0].split('?')[0]

                # For ALL batch sources (GeoPackage, shapefile dirs, databases, etc.),
                # find all layers from the same source
                ...
                layers_from_source = [
                    ...
                    if layer.source().split('|')[0].split('?')[0] == source_path
                ]
                ...
                if source_path in uploaded_sources:
                    ...
                    for batch_layer in layers_from_source:
                        self.main.log_message(
                            f"⏭️ Skipping layer '{batch_layer.name()}' - datastore "
                            f"already loaded, no additional upload required")
                ...
                if len(layers_from_source) > 1:
                    self.main.log_message(
                        f"📦 BATCH UPLOAD: Uploading {len(layers_from_source)} layers from source: {source_path}")
                    self.main.log_message(
                        f"   Layers: {', '.join([layer.name() for layer in layers_from_source])}")
                ...
                for check_layer in layers_to_check:
                    ...
                    layer_exists = self.main._layer_exists_in_workspace(
                        check_layer_name, workspace, url, username, password)

                    if layer_exists:
                        # Layer exists - check if we should overwrite
                        if self.main.auto_overwrite_checkbox.isChecked():
                            self.main.log_message(
                                f"🔄 Auto-overwrite enabled: Deleting existing layer "
                                f"'{check_layer_name}' before re-upload...")
                            ...
                            if not hasattr(self.main, '_overwrite_decision'):
                                overwrite_action = self.main._ask_overwrite_existing_layer(
                                    check_layer_name, i + 1, total_layers)
                            ...
                            if overwrite_action in ['skip', 'skip_all']:
                                self.main.log_message(
                                    f"⚠ Layer '{check_layer_name}' already exists in workspace "
                                    f"'{workspace}'. Skipping as requested.", level=Qgis.Warning)
                        ...
                        self.main.log_message(
                            f"✓ Layer '{check_layer_name}' does not exist in workspace. Proceeding with upload.")
                ...
                for cleanup_layer in layers_to_check:
                    ...
                    self.main._cleanup_all_duplicate_stores_for_layer(
                        cleanup_layer_name, workspace, url, username, password)

                # For batch uploads (multiple layers from same source), upload ONCE
                if len(layers_from_source) > 1:
                    self.main.log_message(
                        f"📦 BATCH UPLOAD: Processing {len(layers_from_source)} layers from single source")
                    ...
                    if is_geopackage:
                        ...
                        self.main.log_message(
                            f"📦 GeoPackage batch upload detected - uploading all "
                            f"{len(layers_from_source)} layers together")
                        success = self.main.geopackage_native_uploader.upload_geopackage_batch(
                            layers_from_source, workspace, url, username, password)
                        ...
                        if success:
                            ...
                            for batch_layer in layers_from_source:
                                if batch_aborted:
                                    self.main.log_message(
                                        f"⏭️ Skipping '{batch_layer.name()}' - batch cancelled by user")
                                    continue
                                batch_layer_name = self.main._sanitize_layer_name(batch_layer.name())
                                continue_batch = upload_processor.handle_upload_success(
                                    batch_layer, batch_layer_name, workspace, url, username, password)
                                if continue_batch is False:
                                    batch_aborted = True
                                    self.main.log_message("🛑 Batch upload cancelled by user")
                                    continue
                        ...
                        self.main.log_message(
                            f"📦 Batch upload: Uploading {len(layers_from_source)} layers individually")

                        batch_aborted = False
                        for batch_layer in layers_from_source:
                            if batch_aborted:
                                self.main.log_message(f"⏭️ Skipping '{batch_layer.name()}' - batch cancelled by user")
                                continue
                            batch_layer_name = self.main._sanitize_layer_name(batch_layer.name())
                            self.main.log_message(
                                f"📤 Uploading batch layer: '{batch_layer.name()}' ({batch_layer_name})")

                            success = upload_processor.process_upload(
                                batch_layer, batch_layer_name, workspace, url, username,
                                password, upload_method, native_format)
                    ...
                    success = upload_processor.process_upload(
                        layer, layer_name, workspace, url, username, password, upload_method, native_format)
                    ...
                    if success:
                        ...
                        if continue_batch is False:
                            self.main.log_message("🛑 Upload cancelled by user")
                            break  # Exit the layer loop
                    else:
