...
from qgis.PyQt.QtWidgets import QTreeWidgetItem, QPushButton, QHeaderView, QLabel
...
class QGISLayersPopulator:
    ...
    def _setup_header_columns(self):
        ...
        header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)  # Rule Based
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)  # Format
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)  # Properties
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # Show SLD
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents)  # Upload SLD
    ...
    def _on_group_name_changed(self, group_node, group_item, old_path):
        ...
        try:
            if not group_node:
                self.main.log_message("❌ Invalid group node for rename")
                return
    ...
    def _setup_layer_buttons(self, item, layer):
        ...
        Args:
            ...
            layer: QGIS layer object
        """
        # Column 1 is Rule Based checkbox
        self._set_rule_based_checkbox(item, layer)

        # Add extents button (column 3 - Properties)
        ...
        btn_extents.clicked.connect(lambda checked, lyr=layer, i=item: self._on_button_clicked_preserve_selection(
            lyr, i, self.main._on_show_extents_clicked))
        self.main.qgis_layers_tree.setItemWidget(item, 3, btn_extents)

        # Add SLD button (applies to both vectors and rasters) (column 4 - Show SLD)
        ...
        btn_show.clicked.connect(lambda checked, lyr=layer, i=item: self._on_button_clicked_preserve_selection(
            lyr, i, self.main._on_show_sld_clicked))
        self.main.qgis_layers_tree.setItemWidget(item, 4, btn_show)

        # Add Upload SLD button (column 5 - Upload SLD)
        ...
        btn_upload_sld.clicked.connect(lambda checked, lyr=layer, i=item: self._on_button_clicked_preserve_selection(
            lyr, i, self.main._on_upload_sld_clicked))
        self.main.qgis_layers_tree.setItemWidget(item, 5, btn_upload_sld)

    def _set_rule_based_checkbox(self, item, layer):
        """
        Set the Symbol column indicator for a layer.
        Shows 'M' (red) for multi-layer symbol rule-based layers,
        'S' (green) for single-layer symbol rule-based layers,
        or blank for non-rule-based layers.

        Args:
            item: Tree widget item
            layer: QGIS layer object
        """
        is_rule_based = self._is_rule_based_symbology(layer)

        if not is_rule_based:
            label = QLabel("")
            label.setToolTip("Standard symbology — single SLD upload")
        elif self._has_multi_layer_symbol(layer):
            label = QLabel("<b>M</b>")
            label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            label.setStyleSheet("color: #006600;")
            label.setToolTip(
                "Multi-layer symbol detected — will flatten each symbol layer "
                "into its own draw pass in the GeoServer Style Group."
            )
        else:
            label = QLabel("<b>S</b>")
            label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            label.setStyleSheet("color: #006600;")
            label.setToolTip(
                "Rule-based symbology (single-layer symbols) — "
                "will upload one SLD per rule + GeoServer Style Group"
            )
        self.main.qgis_layers_tree.setItemWidget(item, 1, label)

    def _is_rule_based_symbology(self, layer):
        """
        Check if a layer uses rule-based or merged feature renderer symbology.

        Args:
            layer: QGIS layer object

        Returns:
            bool: True if layer uses rule-based symbology, False otherwise
        """
        ...
        # Check for rule-based renderer types
        rule_based_types = [
            'RuleRenderer',           # Standard rule-based renderer
            'ruleRenderer',           # Alternative naming
            'mergedFeatureRenderer',  # Merged feature renderer
            'MergedFeatureRenderer',  # Alternative naming
        ]

        return renderer_type in rule_based_types

    def _has_multi_layer_symbol(self, layer):
        """
        Return True if any active leaf rule in a rule-based renderer
        uses a multi-layer symbol (symbol with more than one symbol layer).
        ...
        try:
            ...
            def _check_rule(rule):
                if rule.symbol() is not None and rule.symbol().symbolLayerCount() > 1:
                    return True
                for child in rule.children():
                    if child.active() and _check_rule(child):
                        return True
                return False

            return _check_rule(renderer.rootRule())
        except Exception:  # nosec B110
            return False
    ...
    def _connect_layer_signals(self, layer, item):
        ...
        if layer.id() not in self.main.connected_layers:
            ...
            layer.nameChanged.connect(lambda lyr=layer: self._on_layer_name_changed(lyr))

            # Connect renderer/style change signal
            if hasattr(layer, 'rendererChanged'):
                layer.rendererChanged.connect(lambda lyr=layer: self._on_layer_renderer_changed(lyr))
    ...
    def _set_layer_format_info(self, item, layer):
        ...
        Args:
            ...
            layer: QGIS layer object
        """
        # Set format in column 2 (after Rule Based in column 1)
        format_info = get_layer_provider_info(layer)
        item.setText(2, format_info.get('native_format', 'Unknown'))
