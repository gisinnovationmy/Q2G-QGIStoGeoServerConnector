...
class DuplicateDatastoresCleaner:
    ...
    def cleanup_duplicate_datastores(self, layer_name, workspace, url, username, password):
        ...
        try:
            ...
            deleted_count = self._delete_duplicate_and_temporary_datastores(
                datastores, layer_name, workspace, url, username, password)
    ...
    def _fetch_workspace_datastores(self, workspace, url, username, password):
        ...
        response = requests.get(datastores_url, auth=(username, password), timeout=30)
        ...
        for store in datastores:
            ...
            if self._is_duplicate_datastore_with_suffix(store_name, sanitized_layer):
                if self._delete_datastore(store_name, workspace, url, username, password,
                                          "duplicate datastore with suffix"):
                    deleted_count += 1
