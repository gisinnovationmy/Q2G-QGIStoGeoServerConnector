...
class DebugDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.tsv_output.setStyleSheet(
            "QTextEdit { font-family: Consolas; font-size: 10px; background: #1e1e1e; color: #00ff00; }")
    ...
    def _run_diagnostic_tests(self):
        ...
        tests = [
            ...
            ("Qt Resources", "qtwebengine_resources.pak present",
             lambda: self._check_resource_file("qtwebengine_resources.pak")),
            ("Qt Resources", "qtwebengine_resources_100p.pak present",
             lambda: self._check_resource_file("qtwebengine_resources_100p.pak")),
            ("Qt Resources", "qtwebengine_resources_200p.pak present",
             lambda: self._check_resource_file("qtwebengine_resources_200p.pak")),
            ...
            ("Environment", "QTWEBENGINE_DISABLE_SANDBOX",
             lambda: os.environ.get("QTWEBENGINE_DISABLE_SANDBOX", "Not set")),
            ("Environment", "QTWEBENGINE_CHROMIUM_FLAGS",
             lambda: os.environ.get("QTWEBENGINE_CHROMIUM_FLAGS", "Not set")),
    ...
    def _check_sandbox(self):
        ...
        try:
            p = subprocess.Popen([exe], stdout=subprocess.PIPE, stderr=subprocess.PIPE)  # nosec B603
            time.sleep(0.5)
    ...
    def _run_external_script(self, script_name, timeout=15):
        ...
        try:
            ...
            # Run the script without shell=True to avoid command injection risks
            if os.name == "nt":  # Windows
                cmd = ["cmd", "/c", script_path]
            else:  # Linux/macOS
                # Make sure script is executable (owner only)
