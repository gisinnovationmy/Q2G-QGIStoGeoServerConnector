...
class UploadProcessor:
    ...
    def process_upload(self, layer, layer_name, workspace, url, username, password, upload_method, native_format):
        ...
        if native_format == 'GeoPackage' and upload_method == 'gpkg_native':
            self.main.log_message(
                "Routing GeoPackage through importer instead of gpkg_native for better compatibility")
    ...
    def _process_postgis_upload(self, layer, layer_name, workspace, url, username, password):
        ...
        success = self.main._register_postgis_datastore_and_publish(
    ...
    def _process_geopackage_upload(self, layer, layer_name, workspace, url, username, password):
        ...
        success = self.main._register_geopackage_datastore_and_publish(
            layer, layer_name, workspace, url, username, password)
        return success
    ...
    def handle_upload_success(self, layer, layer_name, workspace, url, username, password):
        ...
        try:
            sld_was_counted = self.main.sld_style_uploader.upload_sld_style(
                layer, layer_name, workspace, url, username, password,
                cased_lines=self.main.cased_lines_checkbox.isChecked()
            )
            # Log SLD status but don't count it again (layer already counted above)
            if sld_was_counted is None:
                self.main.log_message("📊 SLD: Upload cancelled by user - aborting batch")
                return False  # Signal to abort batch
            elif sld_was_counted:
                self.main.log_message("📊 SLD: Existing style overwritten")
            else:
                self.main.log_message("📊 SLD: New style created")
        except Exception as e:
            self.main.log_message(f"Warning: Failed to upload SLD for '{layer.name()}': {e}", level=Qgis.Warning)
        return True  # Continue with batch
