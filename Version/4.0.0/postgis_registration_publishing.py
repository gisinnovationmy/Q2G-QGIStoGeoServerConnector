...
class PostGISRegistrationPublisher:
    ...
    def _extract_connection_parameters(self, layer):
        ...
        try:
            ...
            if not all_saved:
                self.main.log_message("✗ No saved PostGIS connections found in postgis.ini")
                return None
            ...
            for cred in all_saved:
                if (cred['database'] == pg_database
                    and cred['host'] == pg_host
                        and int(cred['port']) == pg_port):
                    exact_match = cred
                    break
            ...
            if db_match:
                ...
                self.main.log_message(
                    f"⚠️  No exact match found. Using database name match: "
                    f"{pg_database}@{db_match['host']}:{db_match['port']}")
                return pg_params

            # Strategy 3: Try all saved connections (for different database names)
            self.main.log_message(
                f"⚠️  No database name match found. Trying all {len(all_saved)} saved connection(s)...")
            for idx, cred in enumerate(all_saved, 1):
                self.main.log_message(
                    f"   Attempt {idx}/{len(all_saved)}: {cred['database']}@{cred['host']}:{cred['port']}")
                ...
                if self._verify_table_exists(pg_params):
                    self.main.log_message(
                        f"✓ Successfully connected using: {cred['database']}@{cred['host']}:{cred['port']}")
                    return pg_params
                else:
    ...
    def _create_or_verify_datastore(self, ds_name, pg_params, workspace, url, username, password):
        ...
        ds_resp = requests.get(ds_url, auth=(username, password), timeout=30)
    ...
    def _verify_existing_datastore(self, ds_resp, ds_name, pg_params, workspace, url, username, password):
        ...
        try:
            ...
            self.main.log_message(
                f"Existing datastore connection parameters: "
                f"host={existing_params.get('host')}, "
                f"database={existing_params.get('database')}, "
                f"user={existing_params.get('user')}")

            # Check if the existing datastore has valid connection parameters
            if not existing_params.get('host') or not existing_params.get(
                    'database') or not existing_params.get('user'):
                self.main.log_message("Existing datastore has invalid connection parameters. Updating datastore...")
    ...
    def _update_datastore(self, ds_name, pg_params, workspace, url, username, password):
        ...
        if update_resp.status_code in [200, 201]:
            ...
            delete_resp = requests.delete(delete_url, auth=(username, password), timeout=30)
    ...
    def _verify_table_exists(self, pg_params):
        ...
        try:
            ...
            test_uri.setConnection(pg_params['host'], str(pg_params['port']),
                                   pg_params['database'], pg_params['user'], pg_params['passwd'])
            ...
            if metadata:
                ...
                if connection:
                    ...
                    if pg_params['table'] not in table_names:
                        self.main.log_message(
                            f"ERROR: Table '{pg_params['table']}' not found in schema "
                            f"'{pg_params['schema']}'. Available tables: {table_names}")
                        return False
                    else:
                        self.main.log_message(
                            f"Verified table '{pg_params['table']}' exists in schema '{pg_params['schema']}'")
                        return True
                else:
                    self.main.log_message("WARNING: Could not verify table existence - connection failed")
                    return True  # Continue anyway
            else:
                self.main.log_message("WARNING: Could not get PostgreSQL provider metadata")
                return True  # Continue anyway
        except Exception as e:
    ...
    def _handle_existing_layer(self, layer_name, workspace, url, username, password):
        ...
        ft_resp = requests.get(featuretype_url, auth=(username, password), timeout=30)

        # Also check if layer exists at workspace level
        layer_url = f"{url}/rest/workspaces/{workspace}/layers/{layer_name}.json"
        layer_resp = requests.get(layer_url, auth=(username, password), timeout=30)
        ...
        if layer_exists_anywhere:
            ...
            if self.main.auto_overwrite_checkbox.isChecked():
                ...
                delete_layer_resp = requests.delete(delete_layer_url, auth=(
                    username, password), params={'recurse': 'true'}, timeout=30)
                if delete_layer_resp.status_code in [200, 204]:
                    ...
                    self.main.log_message(
                        f"Failed to delete existing layer for overwrite: "
                        f"{delete_layer_resp.status_code} {delete_layer_resp.text}")
                    return False
            else:
    ...
    def _create_featuretype(self, layer, layer_name, pg_params, ds_name, workspace, url, username, password):
        ...
        if pub_resp.status_code not in [200, 201]:
            ...
            self.main.log_message(
                f"Failed to publish table '{pg_params['table']}' as '{layer_name}': {pub_resp.status_code}")
            ...
            if "already exists in store" in str(error_detail):
                self.main.log_message(
                    f"ℹ️ Featuretype '{layer_name}' already exists in datastore "
                    f"'{ds_name}'. Proceeding to publish layer.")
    ...
    def _publish_layer(self, layer_name, workspace, url, username, password):
        ...
        if layer_pub_resp.status_code in [200, 201]:
            ...
            self.main.log_message(
                f"❌ Failed to publish layer '{layer_name}': "
                f"{layer_pub_resp.status_code} - {layer_pub_resp.text}", level=Qgis.Critical)
            return False
