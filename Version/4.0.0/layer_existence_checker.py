...
class LayerExistenceChecker:
    ...
    def layer_exists_in_workspace(self, layer_name, workspace, url, username, password):
        ...
        2. Featuretype in PostGIS datastore:
           /rest/workspaces/{workspace}/datastores/postgis_postgres/featuretypes/{layer_name}
    ...
    def check_layer_exists_at_workspace_level(self, layer_name, workspace, url, username, password, log_prefix=""):
        ...
        layer_resp = requests.get(layer_check_url, auth=(username, password), headers={'Accept': 'application/json'}, timeout=30)
    ...
    def check_featuretype_exists_in_postgis_datastore(self, layer_name, workspace, url, username, password):
        ...
        if postgis_response.status_code == 200:
            self.main.log_message(
                f"DEBUG: Found featuretype '{layer_name}' in PostGIS datastore (not yet published as layer)")
            return True

        return False

    def check_featuretype_exists_in_datastore(self, layer_name, datastore_name,
                                              workspace, url, username, password, log_prefix=""):
        """
        ...
        self.main.log_message(
            f"DEBUG: Checking if {log_prefix.lower() if log_prefix else ''}featuretype exists at: {ft_check_url}")
        ft_resp = requests.get(ft_check_url, auth=(username, password), headers={'Accept': 'application/json'}, timeout=30)
