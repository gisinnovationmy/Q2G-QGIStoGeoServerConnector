...
class WMSLayerBBoxRetriever:
    ...
    def get_wms_layer_bbox(self, url, username, password, layer_name):
        ...
        try:
            ...
            root = safe_fromstring(capabilities_xml)
        ...
        try:
            ...
            if response.status_code != 200:
                self.main.log_message(
                    f"Failed to fetch WMS Capabilities: {response.status_code} - Response: {response.text[:200]}...")
                return None
    ...
    def _try_geographic_bounding_box(self, layer_node, layer_name, namespaces):
        ...
        if bbox_node is not None:
            try:
                west = float(bbox_node.findtext('wms:westBoundLongitude',
                             bbox_node.findtext('westBoundLongitude'), namespaces))
                east = float(bbox_node.findtext('wms:eastBoundLongitude',
                             bbox_node.findtext('eastBoundLongitude'), namespaces))
                south = float(bbox_node.findtext('wms:southBoundLatitude',
                              bbox_node.findtext('southBoundLatitude'), namespaces))
                north = float(bbox_node.findtext('wms:northBoundLatitude',
                              bbox_node.findtext('northBoundLatitude'), namespaces))
