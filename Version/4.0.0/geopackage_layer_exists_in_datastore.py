...
class GeoPackageLayerExistenceChecker:
    ...
    def geopackage_layer_exists_in_datastore(self, layer_name, gpkg_datastore_name, workspace, url, username, password):
        ...
        try:
            ...
            if self.main.layer_existence_checker.check_layer_exists_at_workspace_level(
                    layer_name, workspace, url, username, password, "GeoPackage"):
                return True

            # Check if the featuretype exists in the datastore
            if self.main.layer_existence_checker.check_featuretype_exists_in_datastore(
                    layer_name, gpkg_datastore_name, workspace, url, username, password, "GeoPackage"):
                # Try to publish it as a layer
                return self._publish_featuretype_as_layer(
    ...
    def _publish_featuretype_as_layer(self, layer_name, gpkg_datastore_name, workspace, url, username, password):
        ...
        self.main.log_message(
            f"ℹ️ GeoPackage featuretype '{layer_name}' exists but layer doesn't. Publishing featuretype as layer...")
