...
class LayerCopyMoveHandler:
    ...
    def _cleanup_temp_files_on_success(self):
        ...
        """
        print("DEBUG: Cleaning up temporary files after successful operation")
    ...
    def _cleanup_temp_files_on_failure(self):
        ...
        """
        print("DEBUG: Cleaning up temporary files after failed operation")
        self._cleanup_temp_files()
    ...
    def _ensure_style_exists(self, url, auth, source_workspace, target_workspace, style_name):
        ...
        print("DEBUG: Style doesn't exist in target, copying from source")
    ...
    def copy_layer(self, url, auth, source_workspace, layer_name, target_workspace,
        ...
        try:
            ...
            print("DEBUG: Getting layer metadata")
            ...
            if not layer_metadata:
                print("DEBUG: Could not get layer metadata!")
            ...
            if layer_type == 'RASTER':
                print("DEBUG: Raster layer detected - using raster copy method")
            ...
            print("DEBUG: Getting data store config")
            ...
            if sld_content:
                ...
                if success:
                    ...
                    print("DEBUG: Style copied successfully")
                else:
                ...
                print("DEBUG: No SLD content found for layer")

            # Copy layer configuration with actual style name
            print("DEBUG: Copying layer configuration...")
            ...
            print("DEBUG: Layer copied successfully")
    ...
    def _copy_sld_style(self, url, auth, source_workspace, target_workspace, style_name, sld_content):
        ...
        try:
            ...
            if response.status_code not in [200, 201]:
                ...
                return False, f"Failed to create style: {response.status_code} - {error_text}"
    ...
    def _copy_file_based_layer(self, url, auth, source_workspace, layer_name, target_workspace,
        ...
        try:
            ...
            if is_geopackage:
                print("DEBUG: Detected GeoPackage datastore - will download .gpkg file directly")
            ...
            print("DEBUG: Using WFS export/import approach for file-based datastore")
            ...
            print("DEBUG: Exporting data from source layer via WFS")
            wfs_url = f"{url}/wfs"
            wfs_params = {
                'service': 'WFS',
                'version': '2.0.0',
            ...
            print("DEBUG: GeoPackage uploaded successfully")
    ...
    def _copy_geopackage_direct(self, url, auth, source_workspace, layer_name, target_workspace,
        ...
        try:
            ...
            if not response or response.status_code != 200:
                print("DEBUG: All direct download methods failed, trying WFS export")
        ...
        try:
            ...
            if update_response.status_code not in [200, 201]:
                self.main.log_message(
                    f"⚠️ Layer update returned status {update_response.status_code}", level=Qgis.Warning)

            # Clean up temp files on success
            self._cleanup_temp_files_on_success()

            self.log_message(
                f"✓ GeoPackage layer '{layer_name}' copied via WFS to '{target_workspace}' as '{new_layer_name}'")
    ...
    def _copy_raster_layer(self, url, auth, source_workspace, layer_name, target_workspace,
        ...
        try:
            ...
            print("DEBUG: Exporting raster data from source layer via WCS")
            wcs_url = f"{url}/wcs"
            wcs_params = {
                'service': 'WCS',
                'version': '2.0.1',
            ...
            if response.status_code != 200:
                ...
                print("DEBUG: WCS export failed with __ separator, trying : separator")
            ...
            print("DEBUG: GeoTIFF uploaded successfully")
    ...
    def _copy_layer_config(self, url, auth, source_workspace, layer_name, target_workspace,
        ...
        try:
            ...
            if not feature_type or 'featureType' not in feature_type:
                print("DEBUG: No feature type found - this should not happen for vector layers")
                return False, "Could not get feature type info for vector layer"

            # Vector layer - copy feature type and layer
            print("DEBUG: Copying vector layer (feature type)")
            ...
            print("DEBUG: Checking if datastore exists in target workspace")
            ...
            if response.status_code != 200:
                print("DEBUG: Datastore doesn't exist in target, creating it now")
                ...
                print("DEBUG: Datastore already exists in target workspace")
            ...
            print("DEBUG: Updating layer with style and settings")
            ...
            if style_response.status_code != 200:
                ...
                if not success:
                    print("DEBUG: Could not ensure style exists, layer may use default style")
            else:
                print(f"DEBUG: Style {style_name} already exists in target workspace")
            ...
            if response.status_code not in [200, 201]:
                print("DEBUG: Layer update failed, but feature type was created successfully")
