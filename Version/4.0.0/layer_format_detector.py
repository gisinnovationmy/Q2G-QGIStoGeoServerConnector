...
def get_layer_provider_info(layer):
    ...
    # - PostGIS: postgresql://user:<pwd>@host/database?table=tablename
                ...
                if not pg_database or not pg_table:
                    return {
                        ...
                        'error': (
                            ...
                            f'host={pg_host}, database={pg_database}, table={pg_table}'
                        )
                    }
        ...
        if provider_type == 'ogr' and os.path.isdir(source_path):
            ...
            try:
                ...
                if any(f.lower().endswith('.gpkg') for f in dir_contents):
                    if DEBUG_LAYER_FORMAT:
                        print("DEBUG: Found .gpkg files in directory - returning GeoPackage")
                    return {
                        'provider_type': 'ogr',
                        'source': layer.source(),
                        'is_supported_native': True,
                ...
                # Check for shapefile components
                    ...
                    if DEBUG_LAYER_FORMAT:
                        print("DEBUG: Found .shp files in directory - returning Shapefile")
                    return {
                        'provider_type': 'ogr',
                        'source': layer.source(),
                        'is_supported_native': True,
                ...
                # Default to Shapefile for other directory-based layers
                    ...
                    if DEBUG_LAYER_FORMAT:
                        print("DEBUG: No specific files found in directory - defaulting to Shapefile")
                    return {
                        'provider_type': 'ogr',
                        'source': layer.source(),
                        'is_supported_native': True,
            ...
            if DEBUG_LAYER_FORMAT:
                print(
                    f"DEBUG: Not a directory or not ogr provider - "
                    f"provider_type: {provider_type}, isdir: {os.path.isdir(source_path)}")
