    ...
    def validate_and_convert_crs(self, layer):
        ...
        if not layer_crs.isValid():
            self.main.log_message(
                f"⚠️ Layer '{layer.name()}' has no valid CRS. Will use EPSG:4326 as default.", level=Qgis.Warning)
        ...
        if not crs_code or crs_code.strip() == '':
            self.main.log_message(
                f"⚠️ Layer '{layer.name()}' has empty CRS. Will use EPSG:4326 as default.", level=Qgis.Warning)
            return layer, False, None
        ...
        if converted_layer:
            self.main.log_message("✓ Successfully converted layer to EPSG:4326")
            return converted_layer, True, crs_code
        else:
            self.main.log_message(
                f"⚠️ Failed to convert layer CRS. Will attempt upload with original CRS {crs_code}", level=Qgis.Warning)
            return layer, False, crs_code
    ...
    def _convert_layer_to_epsg4326(self, layer, original_crs):
        ...
        try:
            ...
            if isinstance(layer, QgsVectorLayer):
                ...
                self.main.log_message("⚠️ Unknown layer type, cannot convert CRS", level=Qgis.Warning)
                return None
        ...
        try:
            ...
            if error[0] == QgsVectorFileWriter.NoError:
                ...
                if converted_layer.isValid():
                    ...
                    self.main.log_message("❌ Converted layer is invalid", level=Qgis.Critical)
                    return None
            else:
    ...
    def _convert_raster_layer(self, layer, target_crs, original_crs):
        ...
        try:
            ...
            if converted_layer.isValid():
                ...
                self.main.log_message("❌ Converted raster layer is invalid", level=Qgis.Critical)
                return None

        except Exception as e:
            self.main.log_message(f"❌ Error in raster layer conversion: {e}", level=Qgis.Critical)
            self.main.log_message("⚠️ Note: Raster CRS conversion requires GDAL", level=Qgis.Warning)
            import traceback
            self.main.log_message(traceback.format_exc())
            return None
