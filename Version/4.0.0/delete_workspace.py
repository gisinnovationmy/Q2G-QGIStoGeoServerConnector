...
class WorkspaceDeletionManager:
    ...
    def _get_connection_details(self):
        ...
        if self.main.workspaces_list.currentItem():
            workspace_name = self.main.workspaces_list.currentItem().text()
        else:
            workspace_name = None
    ...
    def _handle_deletion_response(self, response, workspace_name):
        ...
        if response.status_code == 200:
            ...
            self.main.log_message(
                f"Failed to delete workspace {workspace_name}: 403 Forbidden. May contain protected resources.")
        else:
            QMessageBox.warning(
                self.main,
                "Error",
            ...
            self.main.log_message(
                f"Failed to delete workspace {workspace_name}: Status {response.status_code} - {response.text}")
